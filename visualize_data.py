...
def plot_snapshot(data, metadata, snapshot_idx, save_path=None):
    ...
    fig.suptitle(f"{metadata['pattern'].capitalize()} Pattern "
                 f"(F={metadata['F']}, k={metadata['k']})", fontsize=14)
...
def plot_time_evolution(data, metadata, num_frames=9, save_path=None):
    ...
    fig.suptitle(f"{metadata['pattern'].capitalize()} Pattern Evolution "
                 f"(F={metadata['F']}, k={metadata['k']})", fontsize=14)
    ...
    try:
        ...
        if args.snapshot is not None:
            if args.snapshot < 0 or args.snapshot >= data['v'].shape[2]:
                print(f"Error: Snapshot index must be between 0 and {data['v'].shape[2] - 1}")
                sys.exit(1)
